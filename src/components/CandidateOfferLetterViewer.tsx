...
import candidateOfferLetterService from '@/services/candidate-offer-letter.service';
import { useToast } from '@/hooks/use-toast';

interface CandidateOfferLetterViewerProps {
...
export const CandidateOfferLetterViewer = ({ 
  applicationId,
  ...
  buttonClassName = '',
  showIcon = true
}: CandidateOfferLetterViewerProps) => {
  ...
  useEffect(() => {
    const checkOfferLetterExists = async () => {
      try {
        const response = await candidateOfferLetterService.getOfferLetterByApplication(applicationId);
        if (response.success && response.offerLetter) {
          setHasOfferLetter(true);
          setOfferLetterId(response.offerLetter.id);
        } else {
  ...
  const checkAndViewOfferLetter = async () => {
    try {
      ...
      if (response.success && response.offerLetter) {
        const offerLetter = response.offerLetter;
        setOfferLetterId(offerLetter.id);
        
        // Get the PDF URL and show in viewer
        const url = await candidateOfferLetterService.getOfferLetterPDF(offerLetter.id);
        setPdfUrl(url);
